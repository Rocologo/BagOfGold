...
public class DisguiseCraftCompat implements Listener {
	...
	public DisguiseCraftCompat() {
		if (isDisabledInConfig()) {
			MobHunting.getInstance().getLogger().info("Compatibility with DisguiseCraft is disabled in config.yml");
		} else {
			mPlugin = Bukkit.getServer().getPluginManager().getPlugin("DisguiseCraft");
			if (mPlugin != null) {
				// dcAPI = DisguiseCraft.getAPI();
				if (mPlugin.getDescription().getVersion().startsWith("1.")
					...
					MobHunting.getInstance().getLogger().info("Your version (" + mPlugin.getDescription().getVersion()
							+ ") of DisguisCraft is too old and not supported by MobHunting.");
				else {
					Bukkit.getPluginManager().registerEvents(this, MobHunting.getInstance());

					MobHunting.getInstance().getLogger().info("Enabling compatibility with DisguiseCraft ("
	...
	private static final DisguiseType aggresiveList[] = { DisguiseType.Blaze, DisguiseType.CaveSpider,
			DisguiseType.Creeper, DisguiseType.EnderDragon, DisguiseType.Enderman, DisguiseType.Endermite,
			DisguiseType.Ghast, DisguiseType.Giant, DisguiseType.Guardian, DisguiseType.PigZombie,
			DisguiseType.Skeleton, DisguiseType.Slime, DisguiseType.Spider, DisguiseType.Witch, DisguiseType.Wither,
			DisguiseType.Zombie };
	...
	private static final DisguiseType passiveList[] = { DisguiseType.Bat, DisguiseType.Chicken, DisguiseType.Cow,
			DisguiseType.Horse, DisguiseType.IronGolem, DisguiseType.MagmaCube, DisguiseType.MushroomCow,
	...
	private static final DisguiseType otherList[] = { DisguiseType.Boat, DisguiseType.EnderCrystal,
			DisguiseType.FallingBlock, DisguiseType.Minecart, DisguiseType.Player, DisguiseType.Villager,
			DisguiseType.Wolf };
